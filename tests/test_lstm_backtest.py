...
def backtest_lstm(symbol="BTCUSDT", window=10, test_size=10):
    bot = Bitbot()
    ...
    for i in range(test_size):
        ...
        predictor = LSTMPredictor(symbol=symbol, window=window)
        predictor.get_last_prices = lambda n=None: train_seq.reshape(-1, 1)
        try:
    ...
    plt.title("Backtest LSTM Predictor")
    plt.legend()
    plt.show()

if __name__ == "__main__":
    backtest_lstm(symbol="BTCUSDT", window=10, test_size=10)
